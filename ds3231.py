...
class DS3231(object):
    ...
    def week(self, week=''):
        if week == '':
            return self.get_reg(DS3231_WDAY_REG)
        else:
            self.set_reg(self.dec2hex(week), DS3231_WDAY_REG)
    ...
    def datetime(self, year='', month='', day='',week='', hour='', min='', sec=''):
        if year == '' or month == '' or day == '' or week == '' or hour == '' or min == '' or sec == '':
            res = []
            res.append(self.year())
            res.append(self.month())
            res.append(self.day())
            res.append(self.week())
            res.append(self.hour())
            res.append(self.min())
            res.append(self.sec())
            return res
